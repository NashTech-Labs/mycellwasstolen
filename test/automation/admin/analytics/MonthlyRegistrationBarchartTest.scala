...
class MonthlyRegistrationBarChartTest extends Specification {
  val port = 19001
  val baseUrl = "http://localhost:19001"
  "Rendering Registrations Bar Chart" in {
    running(TestServer(port, FakeApplication(additionalConfiguration = inMemoryDatabase())), HTMLUNIT) { browser =>
      ...
      driver.findElementById("analytics").click
      driver.findElementById("barChart").click
      driver.findElementByCssSelector("BODY").getText().contains("Monthly Registrations")
    }
  }
}
