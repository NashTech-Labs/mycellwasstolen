...
class PerDayRegistrationTimeSeriesChartTest extends Specification {
  val port = 19001
  val baseUrl = "http://localhost:19001"
  "Rendering Per Day Registration TimeSeries" in {
    running(TestServer(port, FakeApplication(additionalConfiguration = inMemoryDatabase())), HTMLUNIT) { browser =>
      ...
      driver.findElementById("analytics").click
      driver.findElementById("timeSeriesChart").click
      driver.findElementByCssSelector("BODY").getText().contains("Per Day Registration Trends")
    }
  }
}
