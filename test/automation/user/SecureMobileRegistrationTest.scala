...
class SecureMobileRegistrationTest extends Specification {
  val port = 19001
  val baseUrl = "http://localhost:19001"
  "Clean Mobile Registration" in {
    running(TestServer(port, FakeApplication(additionalConfiguration = inMemoryDatabase())), HTMLUNIT) { browser =>
      ...
      driver.findElementById("clean").click()
      ...
      driver.findElementByCssSelector("tbody").getText().contains("clean")
